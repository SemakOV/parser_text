...
def open_file(filename: tp.Union[str, Path]) -> tp.Union[tp.List[str]]:
    """ Открыаем файл и помещаем строки в лист.
...
def sort_date_and_section_name(pars_file_text: tp.List[str]) -> \
    ...
    результата работы регилярного выражения через группы. Ошибка опеределена в ignore, так как проверка на
    несоответсвие результата поиска значению None определена в блоке 'if'.
...
def split_well_description_str(description_str: str) -> tp.List[str]:
    """ Разибваем строку описания вскрытия скважены с установкой дефолтных значений и очищаем от лишних знаков.

    :param description_str: Сырая строка описания без разделения на определенные значения.

    :return: Список c разделенными данными о вскрытии скважены (дефолтные значения отражены как "DEF").
...
def format_well_description_str(split_str_description: tp.List[tp.Any]) -> tp.Tuple[tp.Any, ...]:
    """ Преобразуем значения строки описания вскрытия скважены к необходимому формату.
    ...
    :param split_str_description: Список c разделенными данными о вскрытии скважены.

    :return: Кортеж c разделенными отформатированными данными о вскрытии скважены.
...
def formatting_allocation(dates_sort_dict: tp.Dict[str, tp.List[tp.List[tp.Any]]],
    ...
    от типа аргумента с данными(дополнительно происходит проверка соответсвия типов аргументов).
...
def sorting_to_class(dates_sort_dict: tp.Dict[str, tp.List[tp.List[tp.Any]]],
    ...
    """ Помещаем значения словаря и списка в объекты соответсвующих классов с учетом дефолтных значений.
    ...
    :return: Объект класса DATES с вложенными в атребуты словарем и списком.
...
def formatting_query_arguments(date: str, name: str, state: str) -> tp.Tuple[str, str, str]:
    """ Преобразуем введеные данные запроса к формату, соотвествующему значениям объекта класса.

    :param date: Строка, содержащая дату.

    :param name: Cтрока, содержащая наименование скважены.

    :param state: Cтрока, содержащая значение флага отрытия скважены.
...
def show_query_results(sort_data: DATES, date_key: str, name: str, state: str) -> tp.Union[COMPDAT, COMPDATL, str]:
    """ Производим поиск, и вывод результата запроса к объекту класса(содержащего отсортиррованные данные).

    :param sort_data: Объект, сожержащий отсортированные данные.

    :param date_key: Строка, садержащая дату в необходимом формате.

    :param name: Строка, садержащая наименование в необходимом формате.

    :param state: Строка, содержащая значение флага отрытия скважены в необходимом формате.
